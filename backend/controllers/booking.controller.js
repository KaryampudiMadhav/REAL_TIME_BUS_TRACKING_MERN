...
export const createOnlineBooking = async (req, res) => {
  try {
    ...
    // 2. Determine booking type and perform specific validations
    const serviceType = trip.vehicle_id.service_type;
    ...
    if (serviceType === "Palle Velugu" || serviceType === "Express") {
      ...
      // No seat numbers for these types, but for consistency in Booking model, we might store an empty array
      actualSeatNumbers = [];
    } else {
    ...
    // --- COUPON LOGIC (Simplified for example) ---
    if (couponCode) {
      const coupon = await Coupon.findOne({
        code: couponCode.toUpperCase(),
        isActive: true,
